...
1. Echo 官方文档：https://echo.labstack.com/
2. OpenAPI 3.1 规范：https://spec.openapis.org/oas/v3.1.0
...
**需求描述**：系统应使用 Genkit Go 定义代码生成 Flow，统一管理 Prompt、模型调用和输出解析；通过 Ollama `/api/chat` + Tool Calling（`write_file` 工具）获取模型输出。

#### 3.4.4 需求编号：FR-AI-004

**需求描述**：系统应内置高质量的 System Prompt 模板，在 `/api/chat` 的 `tools` 参数中携带 `write_file` 工具定义。模型通过 `tool_calls` 返回文件写入指令（`path` + `content`），脚手架解析后写入磁盘。Prompt 中应约束代码需使用当前项目指定的框架和 ORM、包含错误处理等。

**说明**：Prompt 中的框架、ORM、数据库等上下文信息应从项目元数据（`.go-scaffold.json`）读取，而不是由用户在每次 `generate` 调用时重复输入。

#### 3.4.5 需求编号：FR-AI-005

**需求描述**：模型返回的 `tool_calls` 应先经过参数完整性校验（`path` 和 `content` 均为非空字符串、路径格式合法），再由 `go/ast`（必要时辅以 `tree-sitter`）进行语法校验。如果任一校验失败，系统应提示用户并允许重新生成。

#### 3.4.6 需求编号：FR-AI-006

**需求描述**：系统应支持指定目标文件路径，将生成的代码写入现有项目的指定位置，或创建新文件。

**说明**：`generate` 命令必须在已初始化的项目目录下执行，系统读取 `.go-scaffold.json` 获取项目上下文后，再按当前项目的技术栈生成代码并写入指定路径。
...
**需求描述**：AI 代码生成模块应支持切换不同的本地模型（如 ornith:9b、qwen3.5:4b），模型配置应可通过配置文件修改。
...
系统通过 HTTP 调用本地 Ollama 的 `/api/chat` 接口进行 AI 代码生成，携带 `write_file` 工具定义。请求和响应格式如下：
...
```json
{
  "model": "ornith:9b",
  "messages": [
    {"role": "system", "content": "你是一个 Go 后端开发专家。使用 write_file 工具为每个新文件调用一次。"},
    {"role": "user", "content": "生成 User 实体的 CRUD 代码，字段包括 name、email。"}
  ],
  "tools": [{
    "type": "function",
    "function": {
      "name": "write_file",
      "description": "创建 Go 源代码文件",
      "parameters": {
        "type": "object",
        "properties": {
          "path": {"type": "string"},
